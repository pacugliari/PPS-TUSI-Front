          ...
          <div class="flex flex-col md:flex-row gap-6">
            ...
            <div class="md:w-2/3 bg-white rounded-lg shadow-md p-4 space-y-6">
              ...
              <div>
                ...
                <div
                  ...
                  *ngIf="vm.profile as profile; else skeletonProfile"
                >
                ...
                <ng-template #skeletonProfile>
                  <div class="grid md:grid-cols-2 gap-4 opacity-60">
                    ...
                    /></mat-form-field>
                    <mat-form-field appearance="outline"
                      ><mat-label>Email</mat-label><input matInput readonly
                    /></mat-form-field>
                    <mat-form-field appearance="outline" class="md:col-span-2"
                      ><mat-label>Teléfono</mat-label><input matInput readonly
                    /></mat-form-field>
                  </div>
                </ng-template>
              </div>
              ...
              <div>
                ...
                >
                  <mat-radio-button value="EFECTIVO"
                    >Efectivo (solo retiro en sucursal)</mat-radio-button
                  >
                  <mat-radio-button value="ONLINE"
                    >Pago online (tarjeta)</mat-radio-button
                  >
                </mat-radio-group>

                @if (vm.metodoPago === 'EFECTIVO' && vm.metodoEnvio ===
                'DOMICILIO') {
                <p
                ...
                <div class="mt-4 grid md:grid-cols-3 gap-4">
                  <mat-form-field appearance="outline" class="md:col-span-1">
                    ...
                    <mat-select
                      [ngModel]="vm.tarjetaSeleccionada"
                      (ngModelChange)="store.setTarjetaSeleccionada($event)"
                    >
                      <mat-option [value]="null" disabled
                        >Seleccioná una tarjeta</mat-option
                      >
                      <mat-option
                        *ngFor="let t of tarjetasDisponibles"
                        [value]="t"
                        >{{ t }}</mat-option
                      >
                    </mat-select>
                  ...
                  <mat-form-field appearance="outline" class="md:col-span-1">
                    ...
                    <input
                      ...
                      pattern="[0-9]*"
                      [ngModel]="vm.ultimos4"
                      (ngModelChange)="store.setUltimos4($event)"
                      placeholder="1234"
                    />
                  </mat-form-field>
                  ...
                  <mat-form-field appearance="outline" class="md:col-span-1">
                    ...
                    <input
                      ...
                      inputmode="numeric"
                      pattern="[0-9]*"
                      [ngModel]="vm.codigoValidacion"
                      (ngModelChange)="store.setCodigoValidacion($event)"
                      placeholder="CVV"
                    />
                  </mat-form-field>
                  ...
                  <div class="md:col-span-3 flex items-center gap-3">
                    <button
                      mat-stroked-button
                      color="primary"
                      (click)="store.validateCard()"
                      [disabled]="
                        !(
                          vm.tarjetaSeleccionada &&
                          vm.ultimos4.length === 4 &&
                          vm.codigoValidacion.length >= 3
                        ) || vm.cardLoading
                      "
                    >
                      @if (vm.cardLoading) { Validando... } @else { Validar
                      tarjeta }
                    </button>

                    @if (vm.cardValid === true) {
                    <span class="text-green-700 text-sm">Tarjeta válida</span> }
                    @if (vm.cardValid === false) {
                    <span class="text-red-600 text-sm">{{
                      vm.cardError || 'Tarjeta inválida'
                    }}</span>
                    }
                  </div>

                  <!-- Promo (auto-aplicada) -->
                  @if (vm.promoDescripcion && vm.promoPercent > 0) {
                  <div
                    class="md:col-span-3 bg-emerald-50 border border-emerald-200 rounded p-3 text-sm"
                  >
                    <b>Promo bancaria aplicada:</b> {{ vm.promoDescripcion }} —
                    {{ vm.promoPercent }}%
                  </div>
                  }
                </div>
              ...
              <div>
                ...
                <mat-radio-group
                  ...
                  class="flex flex-col gap-3"
                >
                  <mat-radio-button value="DOMICILIO"
                    >Envío a domicilio</mat-radio-button
                  >
                  <mat-radio-button value="RETIRO"
                    >Retiro en sucursal</mat-radio-button
                  >
                </mat-radio-group>
                ...
                <div class="mt-4">
                  ...
                  <p class="text-xs text-gray-500 mt-2">
                    Predeterminada:
                    {{
                      etiquetaDireccion(vm.direcciones, vm.selectedAddressId)
                    }}
                  </p>

                  <p
                    *ngIf="!vm.direcciones?.length"
                    class="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-md p-3 mt-2"
                  >
                    No tenés direcciones guardadas. Agregá una desde tu perfil
                    para habilitar el envío a domicilio.
                  </p>
                </div>
                }
              </div>
            ...
            <div class="md:w-1/3 bg-white rounded-lg shadow-md p-4 h-fit">
              ...
              <div class="flex justify-between mb-2">
                <p>Subtotal productos (sin IVA)</p>
                <p>{{ vm.subtotal | currency : 'ARS' : 'symbol-narrow' }}</p>
              </div>

              <div
                class="flex justify-between mb-2"
                [class.opacity-60]="!(vm.descuentoCupon > 0)"
              >
                <p>Descuento cupón</p>
                <p>
                  -{{
                    vm.descuentoCupon || 0 | currency : 'ARS' : 'symbol-narrow'
                  }}
                </p>
              </div>
              ...
              <div
                class="flex justify-between mb-2"
                [class.opacity-60]="!(vm.descuentoPromo > 0)"
              >
                <p>Descuento promo bancaria</p>
                <p>
                  -{{
                    vm.descuentoPromo || 0 | currency : 'ARS' : 'symbol-narrow'
                  }}
                </p>
              </div>
              ...
              <div class="flex justify-between mb-2">
                ...
                <p>
                  ...
                  {{ 0 | currency : 'ARS' : 'symbol-narrow' }} } @else {
                  {{
              ...
              <!-- Mensajes mínimos -->
              @if (vm.metodoPago === 'EFECTIVO' && vm.metodoEnvio !== 'RETIRO')
              {
              <p class="text-xs text-red-600 mt-3">
                El efectivo requiere retiro en sucursal.
              </p>
              } @if (vm.metodoPago === 'ONLINE' && !(vm.tarjetaSeleccionada &&
              vm.ultimos4.length===4 && vm.codigoValidacion.length>=3 &&
              vm.cardValid === true)) {
              <p class="text-xs text-red-600 mt-1">
                Completá y validá los datos de la tarjeta.
              </p>
              }
            </div>
          </div>
...
export class CheckoutComponent {
  ...
  etiquetaDireccion(addresses: Direccion[] = [], id: number | null): string {
    ...
    return `${tag}${d.calle}${d.zona?.ciudad ? ', ' + d.zona.ciudad : ''}${
      d.zona?.provincia ? ', ' + d.zona.provincia : ''
    }${d.cp ? ' (' + d.cp + ')' : ''}`;
  }

  canConfirm(vm: any): boolean {
    if (vm.metodoPago === 'EFECTIVO') {
      return vm.metodoEnvio === 'RETIRO';
    }
    return (
      !!vm.tarjetaSeleccionada &&
      vm.ultimos4?.length === 4 &&
