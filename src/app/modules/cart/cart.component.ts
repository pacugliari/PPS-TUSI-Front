                ...
                <div class="overflow-x-auto">
                        ...
                        <tr class="border-b border-gray-100 last:border-0">
                          <td class="py-4 pr-2">
                            <div class="flex items-center gap-3">
                              <img class="h-16 w-16 md:h-20 md:w-20 rounded object-cover bg-gray-50 ring-1 ring-gray-200"
                                   [src]="line.producto.imagen || 'assets/images/products/default.png'"
                                   [alt]="line.producto.nombre" />
                              <div class="min-w-0">
                                <p class="text-sm md:text-base font-medium line-clamp-2">
                          ...
                          <td class="px-2 py-4 text-center whitespace-nowrap">
                            <span class="text-sm md:text-base">
                              {{ line.producto.precio | currency:'ARS':'symbol-narrow':'1.2-2' }}
                            </span>
                          </td>

                          <td class="px-2 py-4 text-center">
                            <div class="inline-flex items-center">
                              <button class="border border-primary bg-primary text-white hover:bg-transparent hover:text-primary rounded-full w-8 h-8 flex items-center justify-center"
                                      [disabled]="line.cantidad <= 1"
                                      (click)="store.dec(line.producto.idProducto, line.cantidad)"
                                      aria-label="Disminuir cantidad" title="Disminuir">−</button>
                              <span class="w-10 text-center select-none text-sm md:text-base">{{ line.cantidad }}</span>
                              <button class="border border-primary bg-primary text-white hover:bg-transparent hover:text-primary rounded-full w-8 h-8 flex items-center justify-center"
                                      (click)="store.inc(line.producto.idProducto, line.cantidad)"
                                      aria-label="Aumentar cantidad" title="Aumentar">+</button>
                            </div>
                          </td>

                          <td class="px-2 py-4 text-right whitespace-nowrap">
                            <span class="font-medium">
                              {{ line.total | currency:'ARS':'symbol-narrow':'1.2-2' }}
                            </span>
                          </td>
                          ...
                          <td class="px-0 py-4 text-right">
                            <button class="text-gray-400 hover:text-red-600 p-2"
                                    ...
                                    aria-label="Quitar producto" title="Quitar">×</button>
                          </td>
                        </tr>
                      }
                    </tbody>
                  ...
                  <div class="flex flex-col lg:flex-row justify-between items-center gap-3 mt-6">
                    @if (user$ | async; as user) {
                      @if (vm.coupon) {
                        <div class="flex items-center gap-3">
                          <span class="inline-flex items-center gap-2 bg-green-50 text-green-700 border border-green-200 px-3 py-1 rounded-full">
                            Cupón: <b>{{ vm.coupon.code }}</b> ({{ vm.coupon.percent }}%)
                          </span>
                          <button class="bg-white text-red-600 border border-red-500 hover:bg-red-50 rounded-full py-2 px-4"
                        ...
                        <div class="flex items-center">
                          <input #code type="text" placeholder="Código de cupón"
                                 class="border border-gray-300 rounded-l-full py-2 px-4 focus:outline-none" />
                          <button class="bg-primary text-white border border-primary hover:bg-transparent hover:text-primary rounded-r-full py-2 px-4"
...
export class CartComponent {
  ...
  protected readonly user$ = this.global.user$;
